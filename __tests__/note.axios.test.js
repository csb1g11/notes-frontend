...
describe('note action tests', () => {
  ...
  it(`should create action FETCH_NOTES_FULFILLED after successful fetch`, () => {
    ...
    return store.dispatch(
      noteActions.fetchNotes({'token': 'JWT aknfkdnbcd'})
    )
    .then(() => {
  ...
  it(`should create action FETCH_NOTES_REJECTED after 500 error`, () => {
    moxios.wait(() => {
      let request = moxios.requests.mostRecent();
      request.respondWith({
        status: 500,
    ...
    const expectedActions = [
        ...
        payload: { err: { "Reason" : "All the things broke"} } },
    ];
  ...
  it(`should create action NOTIFY_REJECTED after add note request`, () => {
    ...
    const expectedActions = [
        ...
        text: "Oh no! We haven't kept that safe just yet, please try again." },
    ];
  ...
  it(`should create action NOTIFY_SUCCESS after update note request`, () => {
    ...
    return store.dispatch(
      noteActions.saveUpdatedNote(note, user)
    )
    .then(() => {
    	expect(store.getActions()).toEqual(expectedActions);
    });
  ...
  it(`should create action NOTIFY_REJECTED after update note request`, () => {
    ...
    const expectedActions = [
        ...
        phrase: 'hola' },
    	{ type: types.REJECTED_ACTION, 
        text: "Note update not saved, please try again." },
    ];
  ...
  it(`should create action delete note after update note request`, () => {
    ...
    const store = mockStore({ auth: {} });
    return store.dispatch(
      noteActions.deleteNote(note, user)
    )
    .then(() => {
      	expect(store.getActions()).toEqual(expectedActions);
    });
  ...
  it(`should create action delete note NOTIFY_REJECTED after update note request`, () => {
    ...
    const expectedActions = [
        ...
        phrase: 'hola' },
    	{ type: types.REJECTED_ACTION, 
        text: "Couldn't delete that one - please try again." },
    ];
