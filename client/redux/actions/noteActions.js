...
import { FETCH_NOTES_REQUEST, FETCH_NOTES_FULFILLED, FETCH_NOTES_REJECTED, 
  SEARCH_NOTES, ADD_NOTE, ADD_NOTE_REQUEST, DELETE_NOTE, DELETE_NOTE_REQUEST, UPDATE_NOTE, UPDATE_NOTE_REQUEST, 
  CANCEL_UPDATE, SET_UPDATE_NOTE } from './types';
...
export function fetchNotes(user) {
  return dispatch => {
    ...
    return axios.get(config.apiRoot + '/notes/')
      ...
      .catch((err) => {
        dispatch({type: FETCH_NOTES_REJECTED, payload: err.response.data})
      })
  }
}
...
export function saveUpdatedNote(note, user) {
  return dispatch => {
    dispatch({type: UPDATE_NOTE_REQUEST, phrase : note.phrase });
...
export function deleteNote({ phrase, context, definition, language, url}, user) {
  return dispatch => {    
    dispatch({type: DELETE_NOTE_REQUEST, phrase : phrase })
